...
3. **명단 시트** — `create_roster_sheet.bat`을 실행하면 만들어집니다.
   ...
   학생 문자에 선생님을 호칭과 함께 쓰고 싶으면 Teachers 탭 맨 끝
   **Name in Student Texts** 칸(F열)에 표기할 그대로 적으세요 (예:
   `Mr. Joseph`, `Ms. Ciara`). 학생+부모님 문자의 `Teacher:` 줄에만 쓰이고,
   선생님 본인에게 가는 문자의 인사말은 Display Name을 그대로 씁니다.
   비워두면 캘린더 이름이 쓰입니다.
